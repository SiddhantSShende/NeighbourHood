...
func (uc *IntegrationUseCase) ExecuteAction(ctx context.Context, integrationID, userID, action string, params map[string]interface{}) (map[string]interface{}, error) {
	// Parse provider type from integration ID format: "int_<provider>_<userID>"
	parts := strings.SplitN(integrationID, "_", 3)
	if len(parts) < 3 {
		return nil, fmt.Errorf("invalid integration ID format: %s", integrationID)
	}
