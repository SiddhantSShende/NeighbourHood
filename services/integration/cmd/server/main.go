...
import (
	...
	"neighbourhood/services/integration/internal/delivery/grpc/handler"
	"neighbourhood/services/integration/internal/repository/postgres"
	"neighbourhood/services/integration/internal/usecase"
	"neighbourhood/services/integration/pkg/logger"
	"neighbourhood/services/integration/pkg/metrics"
	"neighbourhood/services/integration/pkg/providers"
	"neighbourhood/services/integration/pkg/tracing"
)

func main() {
	// Load configuration
	cfg, err := config.Load("configs/integration.yaml")
	if err != nil {
	...
	registry := providers.NewRegistry(cfg.Providers, log)
	log.Info("Provider registry initialized", "total_providers", registry.Count())

	// Initialize use case
	integrationUseCase := usecase.NewIntegrationUseCase(repo, registry, log)
	...
	integrationHandler := handler.NewIntegrationHandler(integrationUseCase, log)
	pb.RegisterIntegrationServiceServer(grpcServer, integrationHandler)

	// Register health check
	healthServer := health.NewServer()
	grpc_health_v1.RegisterHealthServer(grpcServer, healthServer)
	healthServer.SetServingStatus("integration-service", grpc_health_v1.HealthCheckResponse_SERVING)
	...
	log.Info("Shutting down Integration Service...")

	healthServer.SetServingStatus("integration-service", grpc_health_v1.HealthCheckResponse_NOT_SERVING)
	...
	select {
	case <-stopped:
		log.Info("Integration Service stopped gracefully")
	case <-ctx.Done():
